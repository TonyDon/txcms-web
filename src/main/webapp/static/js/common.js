...
function showLoading(){
	var loadingEl = $("#loading_img");
	var left = $(document).scrollLeft() + 20;
	var top =  $(document).scrollTop() + 20;
	loadingEl.offset({top:top, left:left});
	var mask_bg = $("#mask_bg");
	mask_bg.width($(window).width()-2);
	mask_bg.height($(window).height()-2);
	mask_bg.fadeIn();
	loadingEl.show();
};
...
function showMessage(htmlMsg, w, h){
	$("#message_dialog .msg-content").html(htmlMsg);
	$("#message_dialog").dialog({
		height:w||200,width:h||450,
		buttons:{'关闭': function() {
            $(this).dialog("close");
          }
        }
 ...
 * @param settings
 */
function onGlobalAjaxError(event, xhr, settings, err){
	hideLoading();
	...
	if(window.__dialog) {
		try {
			...
			if(handler && window != handler) {
				handler.onGlobalAjaxError(event, xhr, settings);
				return;
			}
		}catch(e){}
	}
