...
<link href="${ut:getCtxPath()}/static/style/bootstrap.css"
	rel="stylesheet">
<link href="${ut:getCtxPath()}/static/style/bootstrap-theme.css"
	rel="stylesheet">
<title>${supReqInfo.title}-农产品供求信息</title>
<style>
body {
	padding-top: 80px;
	padding-bottom: 40px;
...
<body>
	...
	<div class="container">
		...
		<h4>相关图片</h4>
		<div>
			<div id="rel_images" data-imgs="${supReqInfo.imageUrls}"></div>
		</div>
...
seajs.use(['bootstrap'],function(){
	var relImages = $('#rel_images');
	var imgs = relImages.data('imgs').split('\|');
	var html = [];
	var t = '<img src="'+ctx+'/@img" width="300"/>'
	for(var i in imgs){
		html.push(t.replace(/@img/, imgs[i]));
	}
	relImages.append(html.join('<br/>'));
});
</script>
</body>
</html>